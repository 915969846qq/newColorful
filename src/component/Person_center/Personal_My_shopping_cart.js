...
import { getOrderRequest, getgoodsRequest, getdiaryRequest, addOrderRequest, couponRequest, delcartRequest } from '../../api/cartOrder_api'
...
class Personal_My_shopping_cart extends Component {
    ...
    cartList() {
        ...
        let cartList = this.state.cartarr.map((item, index) => {
            return (<tr className="cart-content-content-tr" key={index}>
                <td>
                    <input type="checkbox" onChange={this.choose.bind(this, index, item.id)}></input>
                </td>
                ...
                <td className="cart-content-content-td-operation">
                    <p onClick={this.shoucang.bind(this, index, item.gid)}>移入收藏</p>
    ...
    clear(){
        this.setState({
            ...
            for(let i=0;i<this.props.cartlist.length;i++){
                this.props.delcart({id:this.props.cartlist[i].gid})
            }
            this.cartList();
        })
    }
    ...
    gopage = () => {
        ...
        console.log(this.state.choosearr.length===0);
        if(this.state.choosearr.length!==0){
            console.log($(".myprovince").val()+'-'+$(".mycity").val()+'-'+$(".mycounty").val());
            let changeaddress = $(".myprovince").val()+'-'+$(".mycity").val()+'-'+$(".mycounty").val();
            this.setState({
                changeaddress:changeaddress
            },()=>{
                this.props.changeaddress(this)
                ...
                <div className="cart-content-content">
                    ...
                    <div className="cart-content-content-footer">
                        <div>
                            ...
                            {/* <span className="cart-content-content-footer-span" onClick={this.choosedel.bind(this)}>删除选中的商品</span> */}
                            <span className="cart-content-content-footer-span" onClick={this.clear.bind(this)}>清空购物车</span>
                        </div>
                        <div>
...
const MapStateToProps = (state, OwnProps) => {
    return {
        ...
        address:state.cartorder.getaddress.data,
    }
...
const MapDispatchToProps = (dispatch) => {
    return {
        ...
        delcart: async (t) => {
            console.log(t)
            const res = await delcartRequest(t)
        ...
        changeaddress: (t) => {
            ...
            dispatch({
                type: 'CHANGEADDRESS',
                changeaddress: t.state.changeaddress
            })
        }
}
};
