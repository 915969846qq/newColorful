...
export default class Decoration_Write_diary extends Component {
  ...
  releaseDiary = () => {
    ...
    if (
      mydata.name === '' ||
      mydata.content === '' ||
      mydata.community === undefined ||
      mydata.decorationCompany === undefined ||
      mydata.style === '' ||
      mydata.builtArea === undefined
    ) {
      notification.open({
        message: '信息有误',
        placement: 'topLeft',
        description: '输入信息不完整,请去人输入内容是否正确',
        onClick: () => {
      ...
      Axios.post('http://47.100.90.56:8080/banJu/Diary/saveDiary', mydata).then(
        (response) => {
          ...
          if (response.status === 200) {
            this.props.history.push('/Personal_Decoration_diary')
          }
        }
      )
    }
