...
import $ from 'jquery'
import { addaddressRequest, addorderRequest, addAddressRequest, updateDataRequest, alipayRequest } from '../../api/cartOrder_api'
...
class Personal_My_shopping_order_two extends Component {
  constructor(props) {
    ...
    this.state = {
      consigneeinfo: {
        aid: this.props.address.id,
        consigneeName: this.props.address.consigneeName,
        address: $(".myprovince").val() + '-' + $(".mycity").val() + '-' + $(".mycounty").val() + '-' + this.props.address.address.split('-')[3],
  ...
  consigneeinfo() {
    let coupon = this.props.couponlist.map((item, index) => {
      return (
    ...
    this.setState({
      ...
      this.setState({
        consigneeinfo: {
          ...
          address: $(".myprovince").val() + '-' + $(".mycity").val() + '-' + $(".mycounty").val() + '-' + $("#area").val(),
  ...
  gopage() {
    ...
    this.setState({
      addaddress: {
        ...
        consigneeName: this.state.consigneeinfo.consigneeName,
        address: this.state.consigneeinfo.address,
        phoneNumber: this.state.consigneeinfo.phoneNumber,
        telephone: this.state.consigneeinfo.telephone,
      }
    }, () => {
      console.log('===============================================')
      let fakeorder = [];
      for (let i = 0; i < this.props.carttoorder.length; i++) {
        let time = parseInt(Math.round(new Date()));
        console.log(this.props.carttoorder);
        console.log('-=-=-=-=-=-=-==-=-=-=-=-=-=-=');
        this.props.addorder({
          uid: JSON.parse(sessionStorage.user).id,
          gid: this.props.carttoorder[i].gid,
          aid: this.props.address.id,
          status: '待付款',
        })//添加订单
        fakeorder.push({
          id:JSON.parse(sessionStorage.user).gid,
          time:time,
          name:this.props.carttoorder[i].name,
          price:this.props.carttoorder[i].price*this.props.carttoorder[i].quantity,
          status: '已付款',
          src:this.props.carttoorder[i].goods,
          logistics_remarks:'正在服务',
        })
      ...
      this.props.fakeorder(fakeorder);//添加订单（假）
      this.props.payable(this);//应付款

      this.props.addaddress(this);//修改地址
      if(this.state.banksrc===''){
        alert('请选择支付银行');
      }else{
        ...
        <div className="ordert-content-content-receiving-content">
          ...
          <div>
            ...
            <input
              ...
              defaultValue={this.props.address.consigneeName}
              onChange={this.change.bind(this, 'consigneeName')}
            />
          </div>
          <div>
            <label>所在地区：</label>
          ...
          <div>
            <label>详细地址：</label>
            <input id="area" onChange={this.change.bind(this, 'area')} disabled={this.state.localflag === 0 ? true : false} defaultValue={this.props.address.address.split('-')[3]}></input>
          </div>
          <div>
            <label>手机号码：</label>
            <input id="phoneNumber" onChange={this.change.bind(this, 'phoneNumber')} disabled={this.state.localflag === 0 ? true : false} defaultValue={this.props.address.phoneNumber}></input>
            <label>固定电话：</label>
            <input id="telephone" onChange={this.change.bind(this, 'telephone')} disabled={this.state.localflag === 0 ? true : false} defaultValue={this.props.address.telephone}></input>
          </div>
          {this.state.consigneeinfo.hasOwnProperty('aid') ? <span onClick={this.savelocal.bind(this)}>请 保 存</span> : <span>已 保 存</span>}
        </div>
      </div>
          ...
          <div>
            ...
            <div className="ordert-content-content-payment-content-footer">
              ...
              <div className="app">
                <div>
                  <label>
                    <input type="radio" name="bank" onClick={this.choosewebbank.bind(this, require('../../assets/images/dingdan/DINGDAN_59.png'))} />
                    <img
                ...
                <div>
                  <label>
                    <input type="radio" name="bank" onClick={this.choosewebbank.bind(this, require('../../assets/images/dingdan/DINGDAN_62.png'))} />
                    <img
                ...
                <div>
                  <label>
                    <input type="radio" name="bank" onClick={this.choosewebbank.bind(this, require('../../assets/images/dingdan/DINGDAN_64.png'))} />
                    <img
...
const MapStateToProps = (state, OwnProps) => {
  return {
    ...
    couponlist: state.cartorder.coupon,
    carttoorder: state.cartorder.carttoorder,
    address: state.cartorder.getaddress.data[0],
...
const MapDispatchToProps = (dispatch) => {
  return {
    ...
    // addaddress: async (t) => {
    //   console.log(t)
    //   const res = await addaddressRequest(t.state.addaddress)
    ...
    addorder: async (t) => {
      console.log(t)
      const res = await addorderRequest(t)
      console.log(res.data)
      dispatch({
        type: 'ADDORDER',
        addorder: res.data
    ...
    updateData: async (t) => {
      // console.log(t)
      const res = await updateDataRequest(t.state.update)
      // console.log(res.data)
      dispatch({
        type: 'UPDATE',
        update: res.data
