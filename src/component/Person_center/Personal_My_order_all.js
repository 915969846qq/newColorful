...
class Personal_My_order_all extends Component {
  ...
  gopage(page, s) {
    ...
    this.setState({
      ...
      this.fakeorderList(this.state.page);
    })
  }
  // 上一页
  lastpage() {
  ...
  nextpage() {
    ...
    console.log(this.props.fakeorder)
    let pages = this.state.page;
    if (pages < Math.ceil(this.props.fakeorder.length / 3)) {
      this.setState({
        page: pages + 1
      }, () => {
        console.log("---------------------------------------")
...
fakeorderList(page) {
  ...
  this.setState({
    endpage: Math.ceil(this.props.fakeorder.length / 3)
  }, () => {
    this.pagesList();
  ...
  let theorder = this.props.fakeorder.slice((page - 1) * 3, page * 3);
  ...
  let TheOrder = theorder.map((item, index) => {
    return (<div className="order-content-content" key={index}>
      <div className="order-content-content-top">
        ...
        <label>订单编号：{this.props.time}</label>

        {/* <span>下单时间：{new Date(parseInt(item.createdDate)).toLocaleString().replace(/:\d{1,2}$/, ' ')}</span> */}
        <span>下单时间：{((new Date(parseInt(this.props.time)).toLocaleString()).split(" "))[0]}</span>
      </div>
      <div className="order-content-content-body">
        <div className="body-left">
          ...
          <div className="body-nit">
            ...
            <p>下单时间：{((new Date(parseInt(this.props.time)).toLocaleString()).split(" "))[0]}</p>
          </div>
        </div>
  ...
  componentWillMount() {
    store.dispatch(async (dispatch) => {
      const res = await getOrderRequest(2)
      dispatch({
        type: 'ORDERLIST',
        allorder: res.data
      }, () => {
  ...
  componentDidMount() {
    ...
    this.setState({
      ...
      this.orderList(1);
      console.log("q3wets4y2hs3e4tazwtab2qg3t5xsnuhbsg3wesrngtj6dmnsfhbrnj,")
      console.log(this.props.fakeorder===undefined)
      if(this.props.fakeorder!==undefined){
        this.fakeorderList(1);
      }
  ...
  del(index, s) {
    ...
    if(this.state.postarr.length!==0){
      ...
      this.props.fakeorder.splice(index + (this.state.page - 1) * 3, 1);
      this.fakeorderList(this.state.page);
    }
  }
  // 添加商品
  add(index, s) {
    if(this.state.postarr.length!==0){
      ...
      console.log(index)
      let thearr = this.props.fakeorder[index];
      console.log(thearr);
      // thearr.createdDate = ((new Date(parseInt(Math.round(new Date()))).toLocaleString()).split(" "))[0]; 
      this.props.fakeorder.push(thearr);
      this.fakeorderList(this.state.page);
    }
...
const MapStateToProps = (state, OwnProps) => {
  return {
    ...
    // allorder: state.cartorder.myorder,
    all: state.cartorder,
    time:state.cartorder.time,
    fakeorder:state.cartorder.fakeorder
  }
}
